...
class System {
   ...
   open() {
      ...
      if (!this.isOpen(row, col)) {
         ...
         if (row > 0) {
            if (this.isOpen(row - 1, col)) { this.union(label, this.toLabel(row - 1, col)); }
         }
         if (col < this.col - 1) {
            if (this.isOpen(row, col + 1)) { this.union(label, this.toLabel(row, col + 1)); }
         }
         if (row < this.row - 1) {
            if (this.isOpen(row + 1, col)) { this.union(label, this.toLabel(row + 1, col)); }
         }
   ...
   union(labelA, labelB) {
      ...
      if (rootA == 'up' && rootB == 'dn') { this.connected = true; }
      else if (rootB == 'up' && rootA == 'dn') { this.connected = true; }
      else if (rootA == 'up' || rootA == 'dn') { this._relation[labelB] = rootA; }
      else if (rootB == 'up' || rootB == 'dn') { this._relation[labelA] = rootB; }
      else { this._relation[labelA] = this.root(labelB); }
   ...
   main() {
      let interval = setInterval(() => {
         ...
         // check percolation
         if (this.numberOfOpenSites > this.row && this.numberOfOpenSites < this.row * this.col) {
            console.log(this.connected);
            if (this.connected) {
               this.print(this.numberOfOpenSites);
